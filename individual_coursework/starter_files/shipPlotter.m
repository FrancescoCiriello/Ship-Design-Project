classdef shipPlotter < matlab.System
    ...
    methods(Access = protected)
        function setupImpl(obj)
            ...
            %  Initialise ship
            obj.ship = plot(obj.ax, [0 obj.L],[0 0],'-','linewidth',4);
            if ~isempty(obj.ship)
            ...
            %  Initialise balls
            cmap = parula(9);
            obj.balls = scatter(obj.ax, zeros(9,1), zeros(9,1), 20, cmap,"filled","MarkerEdgeColor",'k');
            if ~isempty(obj.balls)
                obj.balls.YData = obj.balls.YData;
            end
        ...
        function stepImpl(obj,x,y,theta,locs)
            % Implement algorithm. 
...
%             Check for closed figure
            ...
            obj.ship.XData = [x, x+obj.L*cos(theta)];
            obj.ship.YData = [y, y+obj.L*sin(theta)];

            % update ball location
            obj.balls.XData = locs(1,:);
            obj.balls.YData = locs(2,:);
